...
async fn validation_error_maps_to_400_with_its_message() {
    let resp = error_response(PluginError::ValidationError("page is required".to_string()));
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
    assert_eq!(body_json(resp).await["error"], "page is required");
}
...
#[actix_web::test]
async fn already_installed_maps_to_500_here() {
